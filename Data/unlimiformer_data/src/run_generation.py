...
#     docs = os.listdir("grahamDocs4clusters/")
    # Directory path
    dir_path = "docs/"

    # Regular expression to match files in the format of 'q[number].txt'
    pattern = re.compile(r'^q\d+\.txt$')

    # List only files that match the expected pattern
    docs = [f for f in os.listdir(dir_path) if pattern.match(f)]
    ...
    for x in range(len(docs)//20):
        ...
        for d in docs[x*20: x*20 + 20]:
            if os.path.isfile("docs/" + d):
                with open("docs/" + d, "r") as f:
                    prompt_text = f.read()
                    prompts.append(prompt_text)
        ...
        encoded_prompts = []
        for i in prompts:
            prompt = args.prefix + i + args.suffix
            encoded_prompts.append(tokenizer.encode(prompt, add_special_tokens=False, return_tensors="pt"))
